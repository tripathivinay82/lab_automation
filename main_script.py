...
def setup_private_link(hostname):
    '''
    This function will configure translate, encap and decap profile for Private Link on EXR and DCGW
    '''
    ...
    try:
        with Device(host=hostname, user=username, password=password, normalize=True) as dev:
            ...
            retVal=dev.facts
            if retVal['hostname'] == 'exr02.chg':
                retVal1 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "exr_gre_encap.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal2 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "exr_xlate_profile_v6_to_v4.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal3 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "exr_xlate_profile_v4_to_v6.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal4 = subprocess.run([sys.executable, "decap_inject.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "exr_gre_decap.json"], stdout=subprocess.DEVNULL, timeout=5)
                if retVal1.returncode != 0 or retVal2.returncode != 0 or retVal3.returncode != 0 or retVal4.returncode != 0:
                    print(f"Oops!!Subprocess returned unexpected value...retVal1: {retVal1.returncode}, retVal2:{retVal2.returncode}, retVal3:{retVal3.returncode}, retVal4:{retVal4.returncode}")
                    return False
            elif retVal['hostname'] == 'DCGW':
                retVal1 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "dcgw_gre_encap.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal2 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "dcgw_xlate_profile_v6_to_v4.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal3 = subprocess.run([sys.executable, "route_inject_p3.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "dcgw_xlate_profile_v4_to_v6.json"], stdout=subprocess.DEVNULL, timeout=5)
                retVal4 = subprocess.run([sys.executable, "decap_inject.py", "-H", hostname, "-P", "50051", "-U", username, "-W", password, "-F", "dcgw_gre_decap.json"], stdout=subprocess.DEVNULL, timeout=5)
                if retVal1.returncode != 0 or retVal2.returncode != 0 or retVal3.returncode != 0 or retVal4.returncode != 0:
                    print(f"Oops!!Subprocess returned unexpected value...retVal1: {retVal1.returncode}, retVal2:{retVal2.returncode}, retVal3:{retVal3.returncode}, retVal4:{retVal4.returncode}")
                    return False
            else:
...
def main():
    ...
    isFlexRt = str(input("Do you need Flex Route setup? [Yes/No]: ")) or "No"
    isPl = str(input("Do you need Private Link setup? [Yes/No]: ")) or "No"
    ...
    with multiprocessing.Pool(processes=NUM_PROCESSES) as process_pool: 
        ...
        process_pool.join()
    print(f"retVal {retVal}")
    print("Config Check Function: Multiprocessing Finished in %f sec." % (time.time() - time_start)) 
    ...
    if isFlexRt == 'Yes' or isFlexRt == 'yes':
